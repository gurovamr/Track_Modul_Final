...
def make_network_schematic(model_dir: Path, output_dir: Path):
    ...
    try:
        ...
        print("Warning: Error parsing arterial.csv, skipping fig5")
        return
    
    if not edges:
        print("Warning: No edges found in arterial.csv, skipping fig5")
        return
    ...
    if not finite_distances:
        print("Warning: No finite distances found in graph, skipping fig5")
        return
    ...
    ax.set_aspect('equal')
    ax.axis('off')
    ax.set_title('Network Topology Schematic', fontsize=16, fontweight='bold', pad=20)
    
    fig.tight_layout()
    output_file = output_dir / 'fig5_network_schematic.png'
    plt.savefig(output_file, dpi=150, bbox_inches='tight')
    plt.close()
    
    print("  [5] fig5_network_schematic.png")
...
def main():
    ...
    make_network_schematic(model_dir, output_dir)
